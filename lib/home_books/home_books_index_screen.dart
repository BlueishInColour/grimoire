...
class HomeBooksIndexScreen extends StatefulWidget {
  const HomeBooksIndexScreen({super.key,this.isTablet = false, this.actionBarChild});
...
class _HomeBooksIndexScreenState extends State<HomeBooksIndexScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
     ...
     if(
     Provider.of<MainController>(context,listen: false).openHomeCount ==0
        ...
        Provider.of<MainController>(context,listen: false).openHomeCount = 1;
                            ...
                            Expanded(
                              child: ElevatedButton(
                                  style: ButtonStyle(
                                    ...
                                    fixedSize: WidgetStatePropertyAll(Size(MediaQuery.of(context).size.width,55)),
                                  ...
                                  onPressed: (){
                                    if(!widget.isTablet){
                                      ...
                                      Provider.of<TabletUIController>(context,listen: false).mainSelectedIndex =3;
                                      Provider.of<TabletUIController>(context,listen:  false).secondSelectedIndex =3;
                          ...
                          SliverAppBar(
                            ...
                            IconButton(onPressed: (){
                              ...
                              icon: Badge(
                                backgroundColor: Colors.transparent,
                                label:
                                tabCount(context, future:  LikeRepository().totalCountsOfLikes())  ,
                                child: Icon(EneftyIcons.heart_bold,
                                  color: colorRed,),
                              ),),
                              ...
                              IconButton(onPressed: (){
                                showModalBottomSheet(
                                  ...
                                  icon: Badge(
                                    backgroundColor: Colors.transparent,
                                    label:
                                    tabCount(context, future:  ComingSoonRepository().totalCountsOfSchedules())  ,
...
class FollowingRecommendationView extends StatefulWidget {
  ...
  @override
  State<FollowingRecommendationView> createState() => _FollowingRecommendationViewState();
}

class _FollowingRecommendationViewState extends State<FollowingRecommendationView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body:paginatedView(
          ...
          child: (datas,index){
            FollowModel followModel = FollowModel.fromJson(datas[index].data() as Map<String,dynamic>);
            return paginatedView(
              shrinkWrap: true,
                key: Key("My List"),

                query: FirebaseFirestore.instance.collection("list")
                    .where("createdBy",
                    isEqualTo: followModel.followedAt)
